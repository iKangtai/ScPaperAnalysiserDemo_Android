...
public class SmartPaperMeasureContainerLayout extends FrameLayout {
    ...
    private void drawScanResult(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(scanResult)) {
            ...
            float layerLeft = (getWidth() - scanResultBackgroundWidth) / 2;
            float layerTop = getWidth() / 2 - scanResultBackgroundHeight / 2;
            ...
            float x = (getWidth() - scanResultWidth) / 2;
            float y = getWidth() / 2 + 10;
    ...
    private void drawHint(Canvas canvas) {
        String hint = "请将完整试纸放入镜头内";
        if (!TextUtils.isEmpty(hint)) {
            float hintWidth = textPaint.measureText(hint);
            float x = (getWidth() - hintWidth) / 2;
